...
public class TbMessageSender implements MessageSender {
	...
	public void run() {
		...
		for (Message message : this.messages) {
			try {
				this.sendMessage(StringUtil.trimToEmpty(message.getTargetAddresses()), StringUtil.trimToEmpty(message.getSubject()), StringUtil.trimToEmpty(message.getContent()), StringUtil.trimToEmpty(message.getType().toString()));
			} catch (Exception e) {
				e.printStackTrace();
	...
	private Result sendMessage(String targetAddresses, String subject, String content, String channel) {
		...
		try {
			...
			if (result.isSuccess()) {
				LOG.warn("Send " + channel + " message success, send type is " + sendType + ", targetAddress: " + targetAddresses + ", subject: " + subject + ", content: " + content);
			} else {
				LOG.warn("Send " + channel + " message failure, send type is " + sendType + ", targetAddress: " + targetAddresses + ", subject: " + subject + ", content: " + content);
			}
