			...
			if (!StringUtil.isBlank(cpuUsage) && cpuUsage.endsWith("%")) {
				...
				if (0 < difference) {
					...
					if (GlobalInstance.needAlarm.get()) {
						ThreadPoolManager.addJobToMessageSendExecutor(new TbMessageSender(new Message(wangwangList, "ZK Server cpu usage too high-" + clusterName, hostPerformanceEntity.getIp() + " cpu usage too high! " + cpuUsage + "-" + maxCpuUsage
								+ "=" + difference, Message.MessageType.WANGWANG), new Message(phoneList, "ZK Server cpu usage too high-" + clusterName, "ZK Server cpu usage too high-" + clusterName + hostPerformanceEntity.getIp()
								+ " cpu usage too high! " + cpuUsage + "-" + maxCpuUsage + "=" + difference, Message.MessageType.WANGWANG)));
					}
				}
			}
		}
			...
			if (!StringUtil.isBlank(memoryUsage) && memoryUsage.endsWith("%")) {
				...
				if (0 < difference) {
					...
					if (GlobalInstance.needAlarm.get()) {
						...
						new Message(wangwangList, "ZK Server load usage too high:-" + clusterName, hostPerformanceEntity.getIp() + " load too high：" + memoryUsage + "-" + maxMemoryUsage + "=" + difference, Message.MessageType.WANGWANG),
			...
			if (!StringUtil.isBlank(load)) {
				...
				if (0 < difference) {
					...
					if (GlobalInstance.needAlarm.get()) {
						...
						new Message(wangwangList, " ZK Server load usage too high-" + clusterName, hostPerformanceEntity.getIp() + "：" + load + "-" + maxLoad + "=" + difference, Message.MessageType.WANGWANG),

						new Message(phoneList, "", "ZK Server load usage too high-" + clusterName + hostPerformanceEntity.getIp() + "：" + load + "-" + maxLoad + "=" + difference, Message.MessageType.SMS)
				...
				if (!StringUtil.isBlank(maxDiskUsage)) {
					...
					if (null != diskUsageMap) {
						for (String mountedOn : diskUsageMap.keySet()) {
							...
							if (dataDir.startsWith(StringUtil.trimToEmpty(mountedOn)) || dataLogDir.startsWith(StringUtil.trimToEmpty(mountedOn))) {
								int diskUsage = Integer.parseInt(StringUtil.trimToEmpty(diskUsageMap.get(mountedOn)).replace(PERCENT, EMPTY_STRING));
								if (diskUsage > Integer.parseInt(maxDiskUsage)) {
									LOG.warn("ZK Server " + hostPerformanceEntity.getIp() + " disk usage too high, " + mountedOn + ": " + diskUsage + "%, max setting usage is: " + maxDiskUsage + "%");
									if (GlobalInstance.needAlarm.get()) {
										ThreadPoolManager.addJobToMessageSendExecutor(new TbMessageSender(

										new Message(wangwangList, "ZK Server disk usage too high-" + clusterName, hostPerformanceEntity.getIp() + " disk usage too high, " + mountedOn + ":" + diskUsage + "%, max setting usage is: " + maxDiskUsage
												+ "%", Message.MessageType.WANGWANG),

										new Message(phoneList, "", "ZK disk usage too high-" + clusterName + hostPerformanceEntity.getIp() + "," + mountedOn + ": " + diskUsage + "%, max setting usage is: " + maxDiskUsage + "%",
