...
public class ZKServerStatusCollector implements Runnable {
	...
	private void sshZooKeeperAndHandleStat(String ip, int port, ZooKeeperStatus zooKeeperStatus) {
		...
		try {
			sshResource = SSHUtil.executeWithoutHandleBufferedReader(ip, userNameOfSSH, passwordOfSSH, StringUtil.replaceSequenced(COMMAND_STAT, ip, port + EMPTY_STRING));
			if (null == sshResource) {
	...
	private void telnetZooKeeperAndHandleWchs(String ip, int port, ZooKeeperStatus zooKeeperStatus) {
		...
		try {
			...
			String wchsOutput = SSHUtil.execute(ip, userNameOfSSH, passwordOfSSH, StringUtil.replaceSequenced(COMMAND_WCHS, ip, port + EMPTY_STRING));
	...
	private void sshZooKeeperAndHandleWchc(String ip, int port, ZooKeeperStatus zooKeeperStatus, int clusterId) {
		...
		try {
			...
			String wchcOutput = SSHUtil.execute(ip, userNameOfSSH, passwordOfSSH, StringUtil.replaceSequenced(COMMAND_WCHC, ip, port + EMPTY_STRING));
	...
	private void storeTaoKeeperStatToDB(int clusterId, ZooKeeperStatus zooKeeperStatus) {
		...
		try {
			...
			reportDAO.addTaoKeeperStat(new TaoKeeperStat(clusterId, zooKeeperStatus.getIp(), DateUtil.getNowTime(DateFormat.DateTime), DateUtil.getNowTime(DateFormat.Date), MapUtil.size(zooKeeperStatus.getConnections()),
					zooKeeperStatus.getWatches(), Integer.parseInt(zooKeeperStatus.getSent()), Integer.parseInt(zooKeeperStatus.getReceived()), zooKeeperStatus.getNodeCount()));
		} catch (NumberFormatException e) {
