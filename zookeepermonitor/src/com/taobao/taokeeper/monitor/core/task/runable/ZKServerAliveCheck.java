				...
				if (!StringUtil.isBlank(alarmSettings.getMaxDelayOfCheck())) {
					...
					try {
						...
						if (!sub.checkIfAlive()) {
							if (!sub.checkIfAlive()) { // 连续两次check失败
								...
								if (GlobalInstance.needAlarm.get()) {
									ThreadPoolManager.addJobToMessageSendExecutor(new TbMessageSender(new Message(wangwangList, "ZooKeeper所在机器存活性检测失败" + this.zooKeeperCluster.getClusterName(), "Zk node: " + server + " 存活性检测失败",
											Message.MessageType.WANGWANG)));

									ThreadPoolManager.addJobToMessageSendExecutor(new TbMessageSender(new Message(phoneList, "ZooKeeper所在机器存活性检测失败" + this.zooKeeperCluster.getClusterName(), "Zk node: " + server + " 存活性检测失败",
											Message.MessageType.WANGWANG)));
								}
						...
						if (GlobalInstance.needAlarm.get()) {
							...
							ThreadPoolManager.addJobToMessageSendExecutor(new TbMessageSender(new Message(wangwangList, "ZooKeeper所在机器存活性检测失败" + this.zooKeeperCluster.getClusterName(), "Zk node: " + server + " 存活性检测失败" + e.getMessage(),
									Message.MessageType.WANGWANG)));

							ThreadPoolManager.addJobToMessageSendExecutor(new TbMessageSender(new Message(phoneList, "ZooKeeper所在机器存活性检测失败" + this.zooKeeperCluster.getClusterName(), "Zk node: " + server + " 存活性检测失败" + e.getMessage(),
									Message.MessageType.WANGWANG)));
