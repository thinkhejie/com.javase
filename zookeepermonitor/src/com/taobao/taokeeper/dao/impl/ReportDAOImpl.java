...
public class ReportDAOImpl implements ReportDAO {
	...
	public void addTaoKeeperStat(TaoKeeperStat taoKeeperStat) throws DaoException {
		...
		try {
			String insertSql = StringUtil.replaceSequenced(SqlTemplate.SQL_INSERT_TAOKEEPER_STAT, taoKeeperStat.getClusterId() + "", StringUtil.trimToEmpty(taoKeeperStat.getServer()),
					StringUtil.defaultIfBlank(taoKeeperStat.getStatDateTime(), DateUtil.getNowTime(DateFormat.DateTime)), StringUtil.defaultIfBlank(taoKeeperStat.getStatDate(), DateUtil.getNowTime(DateFormat.Date)), taoKeeperStat.getConnections()
							+ "", taoKeeperStat.getWatches() + "", taoKeeperStat.getSendTimes() + "", taoKeeperStat.getReceiveTimes() + "", taoKeeperStat.getNodeCount() + "");
	...
	public List<TaoKeeperStat> queryTaoKeeperStatByClusterIdAndServerAndStatDate(int clusterId, String server, String statDate) throws DaoException {
		...
		try {
			myResultSet = DbcpUtil.executeQuery(StringUtil.replaceSequenced(SqlTemplate.SQL_QUERY_TAOKEEPER_STAT_BY_CLUSTERID_SERVER_DATE, clusterId, server, statDate));
			if (null == myResultSet) {
				return new ArrayList<TaoKeeperStat>();
			}
	...
	public Map<String, List<TaoKeeperStat>> queryStatByClusterIdAndStatDate(int clusterId, String statDate) throws DaoException {
		...
		DBConnectionResource myResultSet = null;
		try {
			myResultSet = DbcpUtil.executeQuery(StringUtil.replaceSequenced(SqlTemplate.SQL_QUERY_TAOKEEPER_STAT_BY_CLUSTERID_DATE, clusterId, statDate));
			if (null == myResultSet) {
				return CollectionUtil.emptyMap();
			}
			rs = myResultSet.resultSet;
			...
			while (rs.next()) {
				...
				int receiveTimes = rs.getInt("receive_times");
				int nodeCount = rs.getInt("node_count");
				this.addStatToTaokeeperStatMap(taoKeeperStatMap, new TaoKeeperStat(clusterId, server, statDateTime, statDate, connections, watches, sendTimes, receiveTimes, nodeCount));
			}

			return taoKeeperStatMap;

		} catch (Exception e) {
			throw new DaoException("Error when queryTaoKeeperStatByClusterIdAndServerAndStatDate: clusterId:" + clusterId + "statDate:" + statDate + ", Error: " + e.getMessage(), e);
		} finally {
			if (null != myResultSet) {
				DbcpUtil.closeResultSetAndStatement(rs, myResultSet.statement);
				DbcpUtil.returnBackConnectionToPool(myResultSet.connection);
