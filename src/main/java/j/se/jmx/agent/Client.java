...
public class Client {
	...
	public static Date getLocalDate(String expirationDate) throws java.text.ParseException {
		...
		if (expirationDate.endsWith("Z")) {
			...
			if (fmt.getCalendar().getTimeZone().inDaylightTime(date))
				localDate = new Date(localDate.getTime() + fmt.getCalendar().getTimeZone().getRawOffset() + fmt.getCalendar().getTimeZone().getDSTSavings());
			else
	 ...
	 *         the LDAP directory.
	 */
	public static List lookup(DirContext root, String protocolType, String name) throws IOException, NamingException {
		...
		//
		String query = "(&" + "(objectClass=jmxConnector) " + "(jmxServiceURL=*) " + queryProtocol + "(jmxAgentName=" + ((name != null) ? name : "*") + "))";
