...
public class Server {
	 ...
	 */
	public static void register(DirContext root, JMXServiceURL jmxUrl, String name) throws NamingException, IOException {
		...
		if (!oc.contains("jmxConnector")) {
			...
			//
			final String msg = "The supplied node [" + mydn + "] does not " + "contain the jmxConnector objectclass";
		...
		newattrs.put("jmxExpirationDate", getExpirationDate(JMX_DEFAULT_LEASE));
	 ...
	 */
	public JMXConnectorServer rmi(String url) throws IOException, NamingException {
		System.out.println("url : " + url);
		// Make a JMXServiceURL from the url string.
		//service:jmx:rmi://
		//url = url + "localhost" + ":" + 4099;
	...
	public static String getExpirationDate(long sec) {
		...
		if (fmt.getCalendar().getTimeZone().inDaylightTime(date))
			gmtDate = new Date(System.currentTimeMillis() - fmt.getCalendar().getTimeZone().getRawOffset() - fmt.getCalendar().getTimeZone().getDSTSavings() + 1000 * sec);
		else
			gmtDate = new Date(System.currentTimeMillis() - fmt.getCalendar().getTimeZone().getRawOffset() + 1000 * sec);
	...
	public static void main(String[] args) {
		try {
			...
			//
			debug = (Boolean.valueOf(System.getProperty("debug", "true"))).booleanValue();
			...
			System.setProperty(Context.PROVIDER_URL, "ldap://localhost:6000");
			...
			/*
			  service:jmx:rmi://localhost:5000/jndi/rmi://localhost:6000/jmxrmi
			   ...
			   rmi: 这个是connector server的传输协议，在这个url中是使用rmi来进行传输的。
			   JSR 160规定了所有connector server都必须至少实现rmi传输，是否还支持其他的传输协议依赖于具体的实现。
			       比如MX4J就支持soap、soap+ssl、hessian、burlap等等传输协议。
			   localhost:5000: 这个是connector server的IP和端口，该部分是一个可选项，可以被省略掉。（具体参见下文解释）
			   /jndi/rmi://localhost:6000/jmxrmi: 这个是connector server的路径，具体含义取决于前面的传输协议。
			       比如该URL中这串字符串就代表着该connector server的stub是使用jndi api绑定在rmi://localhost:6000/jmxrmi这个地址。
			*/
			final String url = System.getProperty("url", "service:jmx:rmi://");
			//String urlStr = "service:jmx:rmi://localhost/jndi/rmi://" + "localhost" + ":" + 4099 + "/jmxconnector";
			//url = url + "localhost" + ":" + 4099 + "/jmxconnector";
			// Build a JMXServiceURL
			//
			final JMXServiceURL jurl = new JMXServiceURL("service:jmx:rmi://localhost:5000/jndi/rmi://localhost:6000/jmxrmi");
