 ...
 * 大概想法是这样的： 
        生成密钥用于在des算法中加密。 
   classloader类中动态的解密class，并且通过反射机制执行main方法。 
        对classloader类进行高质量的混淆。 
...
public class MyClassLoader1 extends ClassLoader {
	...
	private byte[] loadClassData(String className) throws IOException {
		...
		if (!(filePath == null || filePath == "")) {
			...
			if (!(filePath == null || filePath == "")) {
				System.out.println("It have found the file : " + filePath + ".  Begin to translate");
				...
				System.out.println("Haven't found the file， and fail to read the classData！");
				return null;
			}
		}
	}
	 ...
	 *             If the class could not be found. 
	 */
	protected Class loadClass(String className, String jarName) throws ClassNotFoundException {
		...
		if (!(jarPath == null || jarPath == "")) {
			try {
				...
				while ((entry = in.getNextJarEntry()) != null) {
					String outFileName = entry.getName().substring(entry.getName().lastIndexOf("/") + 1, entry.getName().length());
	...
	public Class reload(String fileName) {
		...
		if (!a.equals(loadClassTime.get(fileName))) {
			...
			System.out.println("The class is the newest version , needn't reloading.");
		}
		return null;
	}
	...
	public String searchFile(String classpath, String fileName) {
		String cut = fileName.substring(fileName.lastIndexOf('.'), fileName.length());
		String path = fileName.substring(0, fileName.lastIndexOf('.')).replace('.', '/') + cut;
		...
		if (f.isFile()) {
			...
			for (int i = 0; i < objects.length; i++) {
				if (new File(classpath + File.separator + objects[i]).isDirectory()) {
					String s = searchFile(classpath + objects[i] + File.separator, fileName);
					if (s == null || s == "") {
						continue;
					} else {
						return s;
